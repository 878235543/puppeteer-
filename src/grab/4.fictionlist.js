...
router.get('/fictionlist', async (req, res) => {
    //更新定时任务
    timedtasksFun();
    res.send({
        status: "OK"
 ...
 */
const timedtasksFun = () => {
    let sql = `select * from timedtasks where title = '百度-小说'`;
    db.query(sql, (err, result) => {
        if (err) {
            console.log("失败!!");
        } else {
            schedule.cancelJob("fictionlist")
            if (result[0].isOpen == 'true') {
                schedule.scheduleJob("fictionlist", result[0].seconds, async () => {
                    console.log('百度-小说--fictionlist:' + new Date());
                    db.query(`truncate table fictionlist`, (err, result) => {
                        if (err) {
                            console.log("清空失败");
                        } else {
                            console.log("清空成功");
                    ...
                    for (let i = 0; i < typeList.length; i++) {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            ...
                            let sql = `insert into fictionlist(classificationType,classificationArea,title,type,actor,context,heat,heatTitle,jumpLink,ranks,img,time,id)
