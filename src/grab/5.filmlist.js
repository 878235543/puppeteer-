 ...
 */
const timedtasksFun = () => {
    let sql = `select * from timedtasks where title = '百度-电影'`;
    db.query(sql, (err, result) => {
        if (err) {
            console.log("失败!!");
        } else {
            schedule.cancelJob("filmlist")
            if (result[0].isOpen == 'true') {
                schedule.scheduleJob("filmlist", result[0].seconds, async () => {
                    console.log('百度-电影--filmlist:' + new Date());
                    db.query(`truncate table filmlist`, (err, result) => {
                        if (err) {
                            console.log("清空失败");
                        } else {
                            console.log("清空成功");
                    ...
                    await page.goto('https://top.baidu.com/board?tab=movie');
                    ...
                    for (let i = 0; i < typeList.length; i++) {
                        ...
                        for (let j = 0; j < areaList.length; j++) {
                            ...
                            for (let i = 0; i < data.length; i++) {
                                ...
                                let sql = `insert into filmlist(classificationType,classificationArea,title,type,actor,context,heat,heatTitle,jumpLink,ranks,img,time,id)
