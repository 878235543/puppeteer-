 ...
 */
const timedtasksFun = () => {
    let sql = `select * from timedtasks where title = '金十数据-快讯2'`;
    db.query(sql, (err, result) => {
        if (err) {
            console.log("失败!!");
        } else {
            schedule.cancelJob("categories2")
            if (result[0].isOpen == 'true') {
                schedule.scheduleJob("categories2", result[0].seconds, async () => {
                    console.log('金十数据-快讯2--categories2:' + new Date());
                    // 启动浏览器
