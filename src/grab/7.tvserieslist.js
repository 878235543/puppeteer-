...
router.get('/tvserieslist', async (req, res) => {
    //更新定时任务
    timedtasksFun();
    res.send({
        status: "OK"
 ...
 */
const timedtasksFun = () => {
    let sql = `select * from timedtasks where title = '百度-电视剧'`;
    db.query(sql, (err, result) => {
        if (err) {
            console.log("失败!!");
        } else {
            schedule.cancelJob("tvserieslist")
            if (result[0].isOpen == 'true') {
                schedule.scheduleJob("tvserieslist", result[0].seconds, async () => {
                    console.log('百度-电视剧--tvserieslist:' + new Date());
                    db.query(`truncate table tvserieslist`, (err, result) => {
                        if (err) {
                            console.log("清空失败");
                        } else {
                            console.log("清空成功");
                    ...
                    await page.goto('https://top.baidu.com/board?tab=teleplay');
                        ...
                        for (let j = 0; j < areaList.length; j++) {
                            ...
                            let data = await page1.evaluate(() => {
                                ...
                                for (let i = 0; i < contentList.length; i++) {
                                    ...
                                    let heatTitleDom = contentList[i].querySelector('.trend_2RttY>.text-index_39kSZ');
                            ...
                            for (let i = 0; i < data.length; i++) {
                                ...
                                let sql = `insert into tvserieslist(classificationType,classificationArea,title,type,actor,context,heat,heatTitle,jumpLink,ranks,img,time,id)
