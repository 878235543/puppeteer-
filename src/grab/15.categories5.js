...
router.get('/categories5', async (req, res) => {
    //更新定时任务
    timedtasksFun();
    res.send({
        status: "OK"
 ...
 */
const timedtasksFun = () => {
    let sql = `select * from timedtasks where title = '金十数据-快讯5'`;
    db.query(sql, (err, result) => {
        if (err) {
            console.log("失败!!");
        } else {
            schedule.cancelJob("categories5")
            if (result[0].isOpen == 'true') {
                schedule.scheduleJob("categories5", result[0].seconds, async () => {
                    console.log('金十数据-快讯5--categories5:' + new Date());
                    // 启动浏览器
