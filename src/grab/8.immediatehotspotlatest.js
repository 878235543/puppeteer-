...
router.get('/immediatehotspotlatest', async (req, res) => {
    //更新定时任务
    timedtasksFun();
    res.send({
        status: "OK"
 ...
 */
const timedtasksFun = () => {
    let sql = `select * from timedtasks where title = '即时热点-最新'`;
    db.query(sql, (err, result) => {
        if (err) {
            console.log("失败!!");
        } else {
            schedule.cancelJob("immediatehotspotlatest")
            if (result[0].isOpen == 'true') {
                schedule.scheduleJob("immediatehotspotlatest", result[0].seconds, async () => {
                    console.log('即时热点-最新--immediatehotspotlatest:' + new Date());
                    // 启动浏览器
                    ...
                    // 在新标签中打开要爬取的网页
                    await page.goto("http://www.jsrank.cn/c/news.html?type=new");
                    ...
                    // console.log(data);
                    db.query('truncate table immediatehotspotlatest', (err, data) => {
                        if (err) {
                            console.log("出错了");
                        } else {
                            addData();
                    ...
                    function addData() {
                        for (let i = 0; i < data.length; i++) {
                            let sql = `insert into immediatehotspotlatest(id,logoImg,logoAlt,title,titleLink,rightTitle,contextListHtml)
